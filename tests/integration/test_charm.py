...
@pytest.mark.parametrize(
    "series",
    [
        (series, pytest.param(series, marks=pytest.mark.group(f"{series}: {GROUP_MARKS[i]}")))
...
@pytest.mark.parametrize(
    "series",
    [
        (series, pytest.param(series, marks=pytest.mark.group(f"{series}: restart 1 unit")))
        for series in TARGET_SERIES
    ],
)
@pytest.mark.abort_on_fail
...
@pytest.mark.parametrize(
    "series",
    [
        (series, pytest.param(series, marks=pytest.mark.group(f"{series}: restart all units")))
        for series in TARGET_SERIES
    ],
)
@pytest.mark.abort_on_fail
...
@pytest.mark.parametrize(
    "series",
    [
        (series, pytest.param(series, marks=pytest.mark.group(f"{series}: stop agent")))
        for series in TARGET_SERIES
    ],
)
@pytest.mark.abort_on_fail
